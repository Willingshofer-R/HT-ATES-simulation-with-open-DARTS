...
class Model(DartsModel):
    ...
    def __init__(self, n_ly, n_cells, dx, dy, dz_array,
                 perm_h, perm_v, poro, hcap, tcond,
                 wells, depth_to_top, geothermal_grad,
        ...
        # #------------Set Node Number------------
        self.n_ly_cap = n_ly[0]  # layer number in cap rock
        self.n_ly_res = n_ly[1] # layer number in reservoir rock
        self.n_ly_bttm = n_ly[2]  # layer number in bottom formation

        self.reservoir = StructReservoir(self.timer, nx=n_cells[0], ny=n_cells[1], nz=n_cells[2],
                                         dx=dx, dy=dy, dz=dz_array,
                                         permx=perm_h, permy=perm_h, permz=perm_v,
                                         poro=poro, start_z=depth_to_top, hcap=hcap, rcond=tcond)

        self.hwx_idx = wells[0, 0]
        self.hwy_idx = wells[0, 1]
        self.cwx_idx = wells[1, 0]
        self.cwy_idx = wells[1, 1]
    ...
    def set_wells(self):
        ...
        for i in range(top_ind, btm_ind + 1):
            self.reservoir.add_perforation("H1",
                                           cell_index= (self.hwx_idx, self.hwy_idx, i),
                                           verbose=True,
                                           multi_segment=False,
                                           well_indexD=0,
                                           skin=10)
            ...
            self.reservoir.add_perforation("L1",
                                           cell_index= (self.cwx_idx, self.cwy_idx, i),
                                           verbose=True,
                                           multi_segment=False,
                                           well_indexD=0,
                                           skin=10)
